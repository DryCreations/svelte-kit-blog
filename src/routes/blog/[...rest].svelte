...
<section class="overflow-hidden">
    <div class="container px-5 sm:pb-24 pb-5 mx-auto">
      ...
      <div class="flex justify-center space-x-1 mt-12 dark:text-gray-400 text-gray-500">
        <a href="{base}/blog/{Math.max(currPage - 1, 0)}/{qs}" class:pointer-events-none={!prev} tabindex={prev ? 0 : -1} class:dark:text-gray-700={!prev} class:text-gray-300={!prev} class:hover:text-gray-900={prev} class:dark:hover:text-white={prev} class=" z-50 flex items-center justify-center h-8 px-2 rounded">
        ...
        {#each pagination as num (num)}
            <a href="{base}/blog/{num}/{qs}" class:text-indigo-600="{num==currPage}" tabindex={num==currPage ? -1 : 0} class:pointer-events-none={num==currPage} class:dark:text-yellow-400="{num==currPage}" class:hover:text-gray-900={num!=currPage} class:dark:hover:text-white={num!=currPage} class:border-t-2="{num==currPage}" class="z-50 rounded-none  dark:border-yellow-400 border-indigo-600 flex items-center justify-center w-8 h-8 text-sm font-medium dark:bg-violet-200 dark:text-violet-500">{num}</a>
        {/each}
        {#if pagination[pagination.length - 1] < numPages - 1}
            <span class="flex items-center justify-center w-8 h-8 rounded">...</span>
        {/if}

        <a href="{base}/blog/{Math.min( numPages - 1,currPage + 1)}/{qs}" class:pointer-events-none={!next} tabindex={next ? 0 : -1} class:dark:text-gray-700={!next} class:text-gray-300={!next} class:hover:text-gray-900={next} class:dark:hover:text-white={next} class="  z-50 flex items-center justify-center h-8 px-2 rounded">
